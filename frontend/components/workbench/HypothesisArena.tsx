        ...
        {(run?.hypotheses || []).map((hypothesis) => (
          <article className="item" key={hypothesis.hypothesis_id}>
            ...
            {hypothesis.revised_statement && (
              <p className="muted">Revision: {hypothesis.revised_statement}</p>
            )}
            {hypothesis.critic && (
              <p className="muted">
                novelty {hypothesis.critic.novelty}/10 · verifiability {hypothesis.critic.verifiability}/10 ·
                evidence {hypothesis.critic.evidence_support}/10 · fit {hypothesis.critic.competition_fit}/10
              </p>
            )}
            ...
            {hypothesis.revision_history[0] && (
              <div className="item-meta">Revision rationale: {hypothesis.revision_history[0].rationale}</div>
            )}

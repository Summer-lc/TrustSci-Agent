...
TrustSci-Agent 是一个基于阿里云百炼 Qwen 的本地优先多智能体 AI Scientist 原型，面向真实文献和外部来源，围绕“检索—引用核验—证据账本—科学假设—受控实验—结果解释—研究报告”建立可追踪科研闭环。V3 主展示领域是地震事件分类，同时保留能源材料经典流程。
...
这里的“本地优先”指运行状态、工作区、实验执行和导出默认保存在本机；Qwen、在线文献源、网页抓取和部分数据服务仍依赖网络。当前百炼远程模型与赛题“国产开源大模型”资格口径是否完全一致，须由团队按正式比赛规则单独确认。
